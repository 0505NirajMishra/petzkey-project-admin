    ...
    <label class="col-lg-2 col-form-label required fw-bold fs-6">{{ trans_choice('content.appt_date_time_title', 1) }}</label>
        ...
        <div class="col-lg-3 fv-row mt-3">
            <select class="form-control form-control-solid" name="progress_status">
              ...
              <option value="0" {{$doctorappoinment->progress_status == 0 ? 'selected':''}} >Upcoming</option>
              <option value="1" {{$doctorappoinment->progress_status == 1 ? 'selected':''}} >Completed</option>
              <option value="2" {{$doctorappoinment->progress_status == 2 ? 'selected':''}} >Canceled</option>
            </select>
        </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.5.0/js/bootstrap-datepicker.js"></script>
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Admin\DoctorAppoinmentRequest', 'form') !!}
