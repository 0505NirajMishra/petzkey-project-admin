...
{{-- @section('admin_filter_form')
    {!! Form::open(['route' => 'admin.bookanappointments.download', 'method' => 'POST', 'id' => 'filter_data', 'class' => 'form mb-15']) !!}
    <!--begin::Card body-->
    <div class="card-body">
        <!--begin::Input group-->
        <div class="row mb-6">
            <!--begin::Label-->
            <label
                class="col-lg-4 col-form-label required fw-bold fs-6">{{ trans_choice('content.bookanappointments.name', 1) }}</label>
            <!--end::Label-->
            <!--begin::Col-->
            <div class="col-lg-8 fv-row">
                {!! Form::text('name', null, ['placeholder' => __('content.bookanappointments.name'), 'value' => 'Max', 'class' => 'form-control form-control-lg form-control-solid mb-3 mb-lg-0 search_input']) !!}
            </div>
            <!--end::Col-->
        ...
        <div class="row mb-6">
            ...
            <label class="col-lg-4 col-form-label fw-bold fs-6">
                <span class="required">{{ trans_choice('content.bookanappointments.id', 1) }}</span>
            </label>
            <!--end::Label-->
            <!--begin::Col-->
            <div class="col-lg-8 fv-row">
                {!! Form::text('bookanappointment_id', null, ['placeholder' => __('placeholder.bookanappointment_id'), 'class' => 'form-control form-control-lg form-control-solid only_number search_input']) !!}
            </div>
            <!--end::Col-->
        </div>
        <!--end::Input group-->
...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.bookanappointment', 2),
        ]),
       
     
        // 'export' => [
        //     'status' => true,
        //     'route' => route('admin.bookanappointments.getdownload'),
        // ],
    ])
    @include('admin.layouts.components.datatable_header', [
        'data' => [
            ['classname' => '', 'title' => trans_choice('content.id_title', 1)],
            ['classname' => 'min-w-125px', 'title' => trans_choice('content.date_title', 1)],
            ['classname' => 'min-w-125px', 'title' => trans_choice('content.start_title', 1)],
            ['classname' => 'min-w-125px', 'title' => trans_choice('content.end_title', 1)],
    ...
    <script>
        ...
        $(document).ready(function() {
            oTable = $('#kt_table_1').DataTable({
                ...
                ajax: {
                    "url": "{{ route('admin.bookanappointments.index') }}",
                    data: function(d) {
                        d.name = $('input[name=name]').val();
                        d.bookanappointment_id = $('input[name=bookanappointment_id]').val();
                ...
                columns: [{
   ...
   
                    {
                        ...
                        if(data==0){
                            ...
                            value_status="Deactive";
                           
                        } 
                        if(data==0){
                        return `<div class="font-medium whitespace-no-wrap" style="color:green">${value_status}</div>`;
                        }
                        else{
                            return `<div class="font-medium whitespace-no-wrap" style="color:red">${value_status}</div>`;
                        }
                    }
                    },
        ...
        $(document).on('click', '.clsdelete', function() {
            ...
            var url = `{{ url('/') }}/admin/bookanappointments/` + id;
        ...
        $(document).on('click', '.clsstatus', function() {
            ...
            var url = `{{ url('/') }}/admin/bookanappointments/status/` + id + `/` + status;
